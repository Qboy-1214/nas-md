...
1. **SSE 广播发送者排除修复**：A 用户编辑后内容重复显示两行的问题。根因是 SSE 广播时使用 `client_id` 排除发送者，但服务器端传入的是 `session_id`，导致排除失败。修复为在 `SSEConnectionHandler` 中存储 `session_id`，广播时使用 `session_id` 进行排除。

2. **版本恢复修复**：从版本历史恢复某版本无效的问题。根因是 `version_history.js` 恢复版本时调用 `window.saveContent()` / `window.debouncedSave()`，但这两个函数不存在。修复为调用 `window.saveFile()` 并 `window.markDirty()` 标记内容已修改。

3. **saveFile 超时机制**：`_saveInProgress` 标志被设置为 true 后没有被正确重置，导致后续保存被跳过。添加 15 秒超时机制，确保 `_saveInProgress` 在超时后自动重置。

4. **客户端信息采集**：扩展 `identity.js` 添加 `parseUserAgent()` 函数，采集操作系统和浏览器信息。版本历史记录显示客户端 IP、操作系统和浏览器，提高用户定位准确性。

5. **文件夹管理功能**：侧边栏新增文件夹重命名和删除按钮（含二次确认），支持本地挂载和服务器挂载操作。

6. **远程文件代理编辑**：新增深链 URL `#remote=<base64url>` 模式，通过 `GET/PUT /api/remote/file` 代理读写局域网其他服务中的 MD 文件。nas-md 不存储文件，版本管理由远程服务负责。
