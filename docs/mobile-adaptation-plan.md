...
#### 1.1 侧边栏体验
- **问题**: sidebar 宽度 280px 在小屏上太宽，文件树文字拥挤
- **方案**:
  - 小屏（< 480px）sidebar 宽度缩为 240px
  - 关闭按钮：在 sidebar 内部右上角添加 ✕ 关闭按钮
  - 点击 overlay 背景关闭 sidebar
  - 文件树项高度在移动端提高到 44px（已做，但需验证）
  - 搜索框在移动端全宽显示

#### 1.2 顶部栏精简
- **问题**: topbar 按钮过多，在手机上放不下
- **方案**:
  - 小屏隐藏非关键按钮（下载、刷新、导出 PDF）
  - 用下拉菜单整合次要操作
  - 关键操作（保存、切换暗色）保留可见
  - 编辑器模式切换按钮组简化为单个按钮 + popover

#### 1.3 编辑器移动端适配
- **问题**: Vditor toolbar 在手机上太拥挤，预览模式体验差
- **方案**:
  - toolbar 改为可滚动横向排列
  - 图标尺寸放大到 44px touch target
  - 分屏预览在手机上改为全高覆盖模式
  - 即时渲染模式在手机上自动全屏
  - 关闭输入法时的预览刷新（避免键盘遮挡）
