...
> 更新日期：2026-06-06
> 当前状态：Phase 1-2 已完成，前端重构 v2 已完成，多用户隔离已实现
...
| `owner` | 创建者的 session UUID | `uuid-xxx`（空字符串表示遗留/宿主机挂载点） |
...
2. **Web 动态挂载**：
   - 所有用户：不限数量（通过 Cookie 会话自动隔离）
   - 动态挂载的目录持久化到 `mounts.json`（按用户分组存储）
...
| 用户 Web 挂载 | ✅ 持久 | `mounts.json` 按用户分组存储，重启保留 |
...
- 挂载目录输入框（路径输入 + 浏览按钮 + "挂载"按钮）
- 已挂载目录列表
- 最近访问的文件列表（按访问时间排序，卸载挂载点自动清理）

> 注：标题、简介、"打开目录"/"新建笔记"/"导入文件"快捷卡片已移除。侧边栏底部"首页"改为"挂载"，"图谱"和"看板"入口已隐藏。
...
- 顶部栏：折叠按钮 + 面包屑 + 编辑器模式切换 + 保存按钮 + 自动保存开关 + 暗色模式切换 + 大纲切换
- 侧边栏：搜索框 + 文件树 + 底部导航按钮
- 主内容区：Vditor 编辑器
...
- [x] 后端：全文搜索（SQLite FTS5）
- [x] 后端：`GET /api/search?q=` 端点
- [x] 后端：YAML frontmatter 解析
- [x] 后端：对象索引器（Page / Task / Tag / Link）
- [x] 后端：双向链接 + 反链
- [x] 后端：结构化查询 API
- [x] 前端：搜索框实时搜索
- [x] 前端：frontmatter 编辑
...
- [x] Phase 1：设计令牌 + 全局样式（Notion 色彩体系、Inter 字体、排版/间距/圆角/阴影变量）
- [x] Phase 2：侧边栏重构（SVG 图标、缩进统一、展开/折叠动画）
- [x] Phase 3：顶部栏 + 编辑器（Vditor CSS 变量覆盖、内容排版）
- [x] Phase 4：模态框 + Toast 动画
- [x] Phase 5：暗色模式 + 响应式布局
- [x] Phase 6：交互优化（键盘快捷键、dirty 状态标记、hover 动画）

### 保存功能增强（已完成）

- [x] Ctrl+S 快捷键保存响应
- [x] 保存按钮保存时置灰效果（保存中 disabled，完成后恢复）
- [x] 自动保存功能（编辑后延迟 3 秒自动保存）
- [x] 自动保存开关（保存按钮旁边，toggle 切换）
- [x] 自动保存不触发保存按钮置灰效果（silent 模式）
...
|--------|------|------|
| Ctrl+K | 聚焦搜索框 | 全局可用 |
| Ctrl+S | 保存文件 | 编辑器聚焦时可用 |
| Ctrl+N | 新建文件 | 全局可用 |
| Escape | 关闭模态框/搜索结果 | 全局可用 |

## 11. 自动保存

- 默认关闭，用户通过开关手动启用
- 开启后，编辑器内容变更后延迟 3 秒自动保存
- 自动保存不触发保存按钮的置灰效果（静默保存）
- 自动保存状态通过 localStorage 持久化
