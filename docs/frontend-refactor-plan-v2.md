...
**2026-06-06 更新：** 以下 UI 变更已实施，审计条目中的部分内容已过时：
- 欢迎页已移除：标题（`.welcome-logo`）、副标题（`.welcome-subtitle`）、描述（`.welcome-desc`）、快捷操作卡片（`.quick-action-card`）
- 侧边栏底部"首页"按钮改为"挂载"，"图谱"和"看板"入口已隐藏
- 挂载目录输入区：移除显示名称输入框（`.open-dir-name-input`），浏览按钮改为 `button-secondary` 样式（transparent 背景），按钮文本改为"挂载"
- 多用户隔离已实现（Cookie 会话 + owner 字段 + 按用户存储挂载点）
- 最近访问列表替代最近修改列表，卸载挂载点时自动清理访问日志
