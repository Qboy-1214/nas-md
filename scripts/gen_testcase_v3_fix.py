...
# Rule 1: P = green
rule_p = CellIsRule(operator='equal', formula=['"P"'], fill=PatternFill(start_color='C6EFCE', end_color='C6EFCE', fill_type='solid'))
ws1.conditional_formatting.add(f'H2:H{len(cases)+1}', rule_p)
# Rule 2: F = red
rule_f = CellIsRule(operator='equal', formula=['"F"'], fill=PatternFill(start_color='FFC7CE', end_color='FFC7CE', fill_type='solid'))
ws1.conditional_formatting.add(f'H2:H{len(cases)+1}', rule_f)
# Rule 3: ? = yellow
rule_q = CellIsRule(operator='equal', formula=['"?"'], fill=PatternFill(start_color='FFEB9C', end_color='FFEB9C', fill_type='solid'))
...
for mod, stats in sorted(module_stats.items()):
    full = 'Yes' if stats['Positive'] > 0 and stats['Negative'] > 0 and stats['Boundary'] > 0 else 'No'
