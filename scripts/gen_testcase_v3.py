...
# Task reward test cases - POSITIVE and BOUNDARY only (no reverse)
for i, row in enumerate(task_rewards):
    ...
    if item3:
        rewards_list.append(f"{item3}×{int(qty3)}")
    if blueprint:
    ...
    # Boundary case: check for zero/missing fields
    ...
    if has_zero or has_missing:
        boundary_parts = []
        if has_missing:
            boundary_parts.append("无道具/蓝图奖励")
        if has_zero:
            boundary_parts.append("存在数量为0的道具")
        ...
        cases.append({
            ...
            'type': '边界',
            'precondition': f'完成{task_desc}',
            'operation': f'完成任务，检查{"，".join(boundary_parts)}场景',
            'expected': '系统正确处理空值/零值，不报错',
            'result': ''
        })
...
for idx, case in enumerate(cases, 2):
    ...
    values = [case['id'], priority, case['module'], case['type'], 
              case['precondition'], case['operation'], case['expected'], case['result']]
...
ws1.conditional_formatting.add(f'H2:H{len(cases)+1}', 
...
ws1.conditional_formatting.add(f'H2:H{len(cases)+1}', 
    CellIsRule(operator='equal', formula=['？'], fill=yellow_fill, font=Font(color='9C6500', bold=True)))
...
title_cell = ws3.cell(row=1, column=1, value='M42 H5主线任务奖励v3 — 黑盒测试用例统计汇总')
...
# Save
output_path = r'C:\Users\Yong\Desktop\OH-WorkSpace\M42_H5主线任务奖励v3_黑盒测试用例.xlsx'
wb.save(output_path)
